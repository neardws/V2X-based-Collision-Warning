...
bool MyUDP::bindPort(QHostAddress addr, qint16 port)
{
    ...
    if (isBinded)  //绑定成功时
    {
        ...
        //如果对方有发送信息过来,QUdpSocket的对象会产生readyRead信号
        qDebug()<<"bind successd";
...
void MyUDP::sendMessage(QHostAddress sender, quint16 senderPort, QJsonObject result)
{
    ...
    // to the host address and at port. sender对方的ip,senderport对方的端口号
...
void MyUDP::readyRead()
{
    ...
    //将包含message信息的信号发送出去,在mainwindow.cpp中与之绑定的槽函数onUdpAppendMessage将会进行调用
